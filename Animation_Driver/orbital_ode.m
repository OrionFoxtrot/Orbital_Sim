function [E_rs, E_vs, t, final_spacecraft] = orbital_ode(spacecraft, time) 
    % Input's Spacecraft (Class), Time (hours)
    % Output's time series E_r using ode45 with acceleration (a) = c.
    c = constants();
    
    X0 = [spacecraft.E_r, spacecraft.E_v];

    t = 0:time*2:time*60*60;
    a = 0;
    
    fdynamic = @(t,X) orbital_step(t,X,a,c,spacecraft);
    options = odeset('RelTol',1e-6,'AbsTol',1e-9);
    [t,X] = ode45(fdynamic, t, X0, options);
    
    E_rs = X(:,1:3);
    E_vs = X(:,4:6);
    final_spacecraft = update_Spacecraft(spacecraft, X(end,1:3)', X(end,4:6)');
...
function Xdot = orbital_step(t,X,a,c,sc)
